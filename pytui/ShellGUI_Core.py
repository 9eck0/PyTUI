...
from EventSystem.FocusLost import FocusLostEventListener
from EventSystem.KeyPress import KeyPressEventHandler,KeyPressEventArgs, KeyCodes
from EventSystem.OnFocus import OnFocusEventListener
...
def addborder(value: str, bordertype=BorderTypes.ThinBorder):
    ...
    :param value: The string representation of the component's layout. Use <component>.value() to get.
    :param bordertype: The border preset to draw with. Defaults to BorderTypes.ThinBorder.
...
class Canvas:
    ...
    def draw(self, delay: float=0, hideoverflown: bool=False, color: str='default'):
        ...
        for index in self.__elem.keys():
            ...
            if hideoverflown:
                if len(LongestFromList(compbuffer)) > (self.Width - comp.width):
            ...
            for lineindex in range(0, len(compbuffer)):
                ...
                #linebuffer[comp.width :] = complinebuffer[0 : len(linebuffer)-comp.X]      - produces undesirable result
                linebuffer[comp.width :] = complinebuffer
                # Update the target 'framebuffer' line with the new one
                framebuffer[comp.height + lineindex]: str = "".join(linebuffer)
...
# 2.0
"""
    Refactored ShellGUI.py into multiple separate modules: ShellGUI_Core.py, Components package
